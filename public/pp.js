...
(function () {
    ...
    function createPoehaliiBadge() {
        ...
        // Try to get project_name from a meta tag
        let projectName = '';
        try {
            ...
            if (projectMetaTag) {
                ...
                console.log('Poehali badge: No project name found in meta tag');
                return null; // Don't show badge if no project name
            }
        } catch (e) {
            console.log('Poehali badge: Error getting project name', e);
            return null;
        }
        
        // Extract project name from hostname if it wasn't found in meta tag
        if (!projectName) {
            try {
                ...
                if (subdomain && subdomain !== 'www' && subdomain !== 'poehali') {
                    ...
                    console.log('Poehali badge: Project name extracted from hostname:', projectName);
                } else {
                    console.log('Poehali badge: Could not extract project name from hostname');
                    return null; // Don't show badge if no project name could be extracted
                }
            } catch (e) {
                console.log('Poehali badge: Error extracting project name from hostname', e);
                return null;
            }
        }
        ...
        try {
            ...
            const url = 'https://devapi.poehali.dev/api/project/badge/displayed?project_name=' + encodeURIComponent(projectName);
            ...
            if (xhr.status === 200) {
                ...
                if (response && response.displayed === false) {
                    console.log('Poehali badge: API returned false for this project');
                    return null; // Don't show badge if API says no
                }
            } else {
                console.log('Poehali badge: API request failed');
                return null; // Don't show badge if API request fails
            }
        } catch (e) {
            console.log('Poehali badge: Error checking badge display status', e);
        ...
        // Add tooltip
        badge.title = "Этот бейдж автоматически скроется при подключении платной подписки на poehali.dev";
